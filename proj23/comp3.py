...
def get_patches(image, kernel=(10, 10), stride=2 , padding=10):
...
def recover(patches, kernel=(10, 10), stride=2, out_shape=(64, 64)):
...
def run_app():
    ...
    # sample_t *= 127
    patches = get_patches(sample_t)
    p[6].write(f'patches: {len(patches)}')
    p[7].write(f'patches shape: {patches.shape}')
    recov = recover(patches)
    recov /= 255
    # recov /= torch.max(recov)
    # while True:
    p[8].write(f'recov: {len(recov)}')
    p[9].write(f'recov shape: {recov.shape}')
    img2 = recov.permute(0, 2, 3, 1).numpy()
    # p[10].image(img2, width=250)
    image_cols[1].image(img2, use_column_width=True)
    flat = (torch.flatten(sample_t) * 255) - 1
    ...
    # **************************************************************************
    flat = (torch.flatten(recov) * 255) - 1
