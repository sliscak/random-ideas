...
oracle = model.NeuralDictionaryV5SDouble(in_features=76, out_features=24, capacity=500)
...
while True:
    ...
    # while learning:
    # while True:
    glob_median_loss = None
    # for x in tqdm(range(10)):
    # if learning:
    old_median_losses = []
    new_median_losses = []
    median_new_loss = None
    while (median_new_loss is None) or (median_new_loss > 20.0):
        old_losses = []
        new_losses = []
        for replay in replay_memory:
            # oracle.zero_grad()
            ...
            new_losses.append(new_loss.detach())
        median_old_loss = np.median(np.array(old_losses))
        median_new_loss = np.median(np.array(new_losses))
        print(f'MEDIAN OLD LOSS: {median_old_loss}\nMEDIAN NEW LOSS: {median_new_loss}')
            # print(f'PRED LOSS: {loss.detach()}\nNEW LOSS: {new_loss}')
