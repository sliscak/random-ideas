...
oracle = model.NeuralDictionaryV9Double(in_features=76, out_features=24, capacity=500)
...
while True:
    ...
    for i_episode in range(1):
        ...
        print(f'LEN: {len(oracle.keys)}')
        print(f'COUNT: {[kvp.count for kvp in oracle.dic.values()]}')
    ...
    # if learning:
        for replay in replay_memory:
            for cap in replay[0]:
                flat = np.concatenate((cap.observation,cap.action))
                oracle.update(torch.tensor(flat, dtype=torch.double), torch.tensor(cap.new_observation, dtype=torch.double))
            # --------------------------------------
            # with torch.no_grad():
            #     new_loss = torch.tensor([0], dtype=torch.double)
            #     for cap in replay[0]:
