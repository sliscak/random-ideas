...
def make_patch_generator(dataset, kernel, stride=1):
    for x in range(0, len(dataset) - kernel[0], stride):
        yield Window(dataset[x:x + kernel[0]], [x,x + kernel[0]])
...
def data2text(data):
    # flat = data.flatten()
    array = [chr(c) for c in data]
    return ''.join(array)

dataset = WikiDataset(dtype=int)
data_chart = st.empty()
patch_chart = st.empty()
...
# data_chart.line_chart(pd.DataFrame(np.reshape(np.array(dataset), (-1, 1))))
for i in range(150, 2, -1):
        ...
        generator = make_patch_generator(dataset, (x, 1), stride=1)
        rows1[0].text(f'TEXT {i}/150')
        for window in generator:
            patch = window.data
            patch_chart.line_chart(pd.DataFrame(np.reshape(np.array(patch), (-1, 1))))
        ...
        st.text(f'EP:{x} LENGTH: {length} MOST COMMON:\n{most}')
        # breakpoint()
