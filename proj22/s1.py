...
class ImageDataset(Dataset):
    def __init__(self, path='C:\Users\Admin\Downloads\i\n01514859\\', testing=False):
...
class Net(nn.Module):
    ...
    def __init__(self, kernel_loc):
        ...
        self.keys = nn.Parameter(torch.randn((50000,900)))
        #kernel shape is 1x4x5x5
        self.values = nn.Parameter(torch.randn((50000, 1*4*5*5)))


        self.keys2 = nn.Parameter(torch.randn((50000, 900)))
        self.values2 = nn.Parameter(torch.randn(50000, 1*1*5*5))
        # self.keys2 = nn.Parameter(torch.randn(50, 900))
        # self.values2 = nn.Parameter(torch.randn(50, 569))
        ...
        for batch in train_loader:
            ...
            for sample in batch:
                ...
                if out_id == target_id.detach().cpu():
                    ...
                    # not byt the number of samples
                    # loss = loss * (1 / len(dataset))
                    loss = loss * (1 / len(dataset.categories))
                else:
                    wrong += 1
                    loss = loss * 1
