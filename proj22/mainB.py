...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.values = nn.Parameter(torch.randn(50, 20))

        self.keys2 = nn.Parameter(torch.randn(50, 20))
    ...
    def forward(self, locations):
        ...
        out = self.values2 * attention
        out = torch.sum(out, dim=0)
        # st.write(f'Special shape: {out.shape}')
...
def run_app():
    ...
    while True:
        ...
        average_glob_loss = glob_loss / len(peace)
        info2.write(f'GlobLOSS: {glob_loss.detach().cpu()}  \nAverageGlobLOSS: {average_glob_loss.detach().cpu()}')
        if average_glob_loss.cpu().detach() < 0.0003:
            slowdown = True
            if first_save is False:
                torch.save(net.state_dict(), PATH)
                first_save = True
                st.write("WEIGHTS SAVED!")
        # average_glob_loss.backward()
        # optimizer.step()

# TODO: Add kornia PSNR loss
