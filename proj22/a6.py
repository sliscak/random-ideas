...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=3, groups=1, kernel_size=(5, 5), stride=2)

        self.keys = nn.Parameter(torch.randn((10, 2700)))
        self.values = nn.Parameter(torch.randn((10, 3*3*5*5)))

        self.keys2 = nn.Parameter(torch.randn((10, 11532)))
        self.values2 = nn.Parameter(torch.randn((10, 3 * 3 * 5 * 5)))

        self.keys3 = nn.Parameter(torch.randn((10, 46875)))
        self.values3 = nn.Parameter(torch.randn((10, 3 * 3 * 5 * 5)))
