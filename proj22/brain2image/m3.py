...
def run_app():
    ...
    for epoch in range(EPOCHS):
        ...
        for image in dataset:
            ...
            out_id = torch.argmax(out, 0).cuda().float()
            target_id = torch.tensor([dataset.categories.index(image.category)]).cuda().float()
            ...
            stats_loc.write(f"OUTPUT:\t{torch.argmax(out.detach().cpu(), 0)}  \nTARGET:\t{target_id}")
            loss = torch.equal(out_id, target_id)
            # print(target.shape)
            # loss = criterion(out, target)
            loss = criterion(out_id, target_id)
            # loss = torch.tensor([0]).cuda()
            # if torch.equal(target_id, out_id):
            #     loss *= 0
            # else:
            #     loss *= 1
