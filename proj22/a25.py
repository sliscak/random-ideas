...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.m = MemC(100, 32*32*3, 128*128*3)
        # self.m2 = MemC(10, 32*32*3, 128*128*3)
    def forward(self, input):
        # input = F.interpolate(input, size=(150, 150))
        input = (2 * input) - 1
        out = self.m(input)
        # out = self.m2(out)
        # out = out + input
        long_f = torch.abs(input - out)
        long_f= torch.sigmoid(long_f)
        long_f = long_f.permute(0, 2, 3, 1)
        long_features.image(long_f.detach().cpu().numpy(), width=250, caption='stable pixels')

        # st.write(f'INPUT: {input.shape}')
        # out = self.lk1(input)
        # out = self.tconv(input)
        # out = self.conv(input)
        # out = torch.relu(out)
        # out = self.conv2(out)
        # out = torch.relu(out)
