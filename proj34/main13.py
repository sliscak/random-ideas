...
model = Net()
model.layer.weight =  torch.nn.Parameter(torch.zeros_like(model.layer.weight))
# model.layer.weight =  torch.nn.Parameter(torch.ones_like(model.layer.weight))
...
for i in range(100000):
    ...
    obs = rgb2gray(obs)
    image = transform.resize(obs, (64, 64))
