...
def init_layout():
    ...
    layout = [
        ...
        [
            sg.Button('Pause', key='pause-button', size=(10, 1), pad=((450, 0), (0, 10), 0, 50), button_color=('black', 'orange')),
            sg.Button('Delete', key='delete-button', size=(10, 1), pad=((0, 0), (0, 10), 0,50), button_color=('white', 'brown')),
        ],
...
if __name__ == '__main__':
    ...
    window = sg.Window(title=APP_NAME, layout=init_layout(), return_keyboard_events=True, use_default_focus=False) # no context manager x(
    with shelve.open(DATABASE_NAME) as db:
        ...
        while True:
            ...
            if ((t - t0) >= 3) and not pause:
                t0 = t
                if len(db.keys()) > 0:
                    weights = [db[key][2]+1 if db[key][3] is not True else 0 for key in db.keys()   ]
            ...
            if (event is not None) or (values is not None):
                if event == chr(13):
                    ...
                    if len(db.keys()) < MAX_ITEMS:
                        ...
                        db[str(r)] = (timestamp, val, priority, done)
                        print(len(db))
                    else:
                    ...
                    print('done')
                    if current_key is not None:
                        v = list(db[current_key])
                        v[3] = True
                        db[current_key] = tuple(v)
                        print(f'Done: K: {current_key} V: {v}')
                        current_key = None
                        mlf = window['multiline-field']
                        mlf.update('')
