...
class Net(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size):
        super().__init__()
        # self.layer1 = Mem(input_size, 1)
        # self.layer2 = Mem(1, 2)
        # self.layer3 = Mem(2, output_size)
...
placeholders_ = [[col.empty() for col in st.beta_columns(2)] for x in range(5)]
placeholders = [[col.empty() for col in st.beta_columns(2)] for x in range(5)]
net = Net(12, 12)
...
while True:
    ...
    placeholders_[0][1].write(meta_df)
